...
for symbol in SYMBOLS :
    ...
    for year in range(1972,2025) :
        ...
        url = "https://in.investing.com/indices/"+symbol+"-historical-data?end_date="+str(end_timestamp)+"&interval_sec=daily&st_date="+str(start_timestamp)
        ...
        main_df = pd.concat([main_df,table_data],axis=0,ignore_index=True)
        
    if symbol == 'volatility-s-p-500' :
        main_df = main_df[main_df['date'] != datetime.datetime.now().strftime('%Y-%m-%d')]

    # main_df.to_csv('Scraped Data/Investing/'+symbol+'.csv',index=False)
    ...
    main_df.to_sql(name=symbol+'_daily',con=engine,if_exists='replace',index=False,chunksize=1000,dtype=columns_type,method='multi')
    engine.dispose()
