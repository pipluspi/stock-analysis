...
for url in URL :
    ...
    soup = BeautifulSoup(html, 'html5lib').find("div", {"id": "ctl00_ContentPlaceHolder1_ctl00_ctl02_Panel1"}).find('tbody')
    ...
    data_present = pd.read_sql(sql='SELECT * FROM `stock-rw-db`.`FED-FUNDS` order by Updated_On desc limit 1',con=engine)
    # print(data_present)
    if (table_data['Actual'][0] != data_present['Actual'][0] ):
        # print(data_present)
        table_data.to_sql(name='FED-FUNDS',con=engine,if_exists='append',index=False,dtype=columns_type,method='multi',chunksize=1000)
